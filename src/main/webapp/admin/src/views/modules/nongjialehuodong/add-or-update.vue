    ...
    >
      <el-row>
        ...
        <el-form-item class="select" v-if="type!='info'"  label="活动类型" prop="huodongleixing">
          <el-select v-model="ruleForm.huodongleixing" placeholder="请选择活动类型">
            <el-option
        ...
        <div v-else>
	      ...
	      <el-input v-model="ruleForm.huodongleixing"
                placeholder="活动类型" readonly></el-input>
                                    ...
                                    <el-col :span="24">  
        <el-form-item class="upload" v-if="type!='info' && !ro.huodongchatu" label="活动插图" prop="huodongchatu">
          <file-upload
          tip="点击上传活动插图"
          action="file/upload"
          :limit="3"
          :multiple="true"
          :fileUrls="ruleForm.huodongchatu?ruleForm.huodongchatu:''"
          @change="huodongchatuUploadChange"
          ></file-upload>
        </el-form-item>
        <div v-else>
          <el-form-item v-if="ruleForm.huodongchatu" label="活动插图" prop="huodongchatu">
            <img style="margin-right:20px;" v-bind:key="index" v-for="(item,index) in ruleForm.huodongchatu.split(',')" :src="item" width="100" height="100">
          </el-form-item>
        </div>
      </el-col>
                                    <el-col :span="12">
...
export default {
  ...
  methods: {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.huodongleixing = true;
            continue;
	            	            ...
	            	            if(o=='huodongchatu'){
            this.ruleForm.huodongchatu = obj[o];
	    this.ro.huodongchatu = true;
            continue;
          }
	            	            if(o=='huodongrenshu'){
      ...
      this.$http({
        ...
        if (data && data.code === 0) {
          ...
          this.$message.error(data.msg);
                                                            ...
                                                            this.huodongleixingOptions = "小型家庭亲自活动".split(',')
                                                                                                                                                                            },
                                                                                        // 多级联动参数
                                                                                                                info(id) {
      this.$http({
        url: `nongjialehuodong/info/${id}`,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
	...
	this.ruleForm.huodongjieshao = this.ruleForm.huodongjieshao.replace(reg,'../../../ssmsi3i6/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
    ...
    onSubmit() {
        ...
        if (valid) {
          this.$http({
            url: `nongjialehuodong/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.nongjialehuodongCrossAddOrUpdateFlag = false;
                  this.parent.search();
    ...
    back() {
      ...
      this.parent.contentStyleChange();
                                                ...
                                                huodongchatuUploadChange(fileUrls) {
                this.ruleForm.huodongchatu = fileUrls;
				this.addEditUploadStyleChange()
            },
                                                                                                	addEditStyleChange() {
	  this.$nextTick(()=>{
